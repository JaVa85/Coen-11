//remember to mutex everything to make sure nothing interferes with each other
...
pthread_t thread;
void *v;
pthread_create(&thread, NULL, cont_save, v);
...
while (1)
{
		...
		default:
			pthread_mutex_lock(&mutex);
			pthread_cancel(thread);		//remember to cancel the thread
			pthread_mutex_unlock(&mutex);
			pthread_mutex_destroy(&mutex);	//get rid of the mutex
...
__fpurge(stdin);		//scan wouldn't work without this
...
void *cont_save(void *a)	//continuously save
{
FILE *fp;
NODE *p;
int i;
...
{
	pthread_mutex_lock(&mutex);	//lock to make sure it saves before something else happens
	fp = fopen("data.bin","wb");	//open to write
	for (i=0; i<4; i++)	//go through each node
	{
		p = head[i];
		while (p != NULL)
		{
			fwrite(p, sizeof(NODE), 1, fp);		//write into the file
			p = p->next;
		}
	}
	fclose(fp);
	pthread_mutex_unlock(&mutex);
	sleep(5);	//pause for five seconds before saving again
}
}

void readit(void)	//binary file func
{
FILE *fp;	//file pointer
NODE temp;

fp = fopen("data.bin", "rb");	//open to read in binary
while (fread(&temp, sizeof(NODE), 1, fp) > 0)	//while something in file
{
	printf("Name: %s\tNumber in Party: %d\n", temp.listnames, temp.listsize);
}
fclose(fp);
}
